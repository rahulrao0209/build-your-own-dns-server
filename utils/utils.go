...
func DecodeDomainName(buf []byte) (string, int) {
	...
	return strings.Join(labels, "."), qTypeIdx
}

// Decodes record type a.k.a QType from a DNS request's question section.
func DecodeType(buf []byte, qTypeIdx int) uint16 {
	return binary.BigEndian.Uint16(buf[qTypeIdx : qTypeIdx+2])
}

// Decodes record class from a DNS's requests question section.
func DecodeClass(buf []byte, qTypeIdx int) uint16 {
	return binary.BigEndian.Uint16(buf[qTypeIdx+2 : qTypeIdx+4])
}
